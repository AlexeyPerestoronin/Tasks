...
impl Solution1 {
    ...
    pub fn multiply(num1: String, num2: String) -> String {
        ...
        let num1: LongDigit = num1.chars().map(|ch| ch as u8).collect();
        let num2: LongDigit = num2.chars().map(|ch| ch as u8).collect();

        let mut result: LongDigit = LongDigit::new();
        num2.into_iter()
            .rev()
            .map(|d2| Self::mul(num1.clone(), d2))
            .for_each(|num1| {result = Self::sum(num1, result.clone());} );

        return num1.iter().map(|ch| char::from(*ch)).collect();
    }
}
